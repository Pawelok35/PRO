...
export default function PowerScoreSelector() {
  ...
  const handleWeightChange = (key, value) => {
    ...
    if (totalWithoutCurrent + newWeight > 100) {
      setWarning(
        `Nie możesz przypisać ${newWeight.toFixed(1)}%. Dostępna pula: ${(100 - totalWithoutCurrent).toFixed(1)}%`
      );
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/calculate_power_score", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="p-4 rounded-xl bg-gray-900 text-white">
      ...
      <div className="mb-4 space-y-2">
        <select value={league} onChange={(e) => setLeague(e.target.value)} className="text-black px-2 py-1">
        ...
        <input placeholder="Drużyna (np. Arsenal)" value={team} onChange={(e) => setTeam(e.target.value)} className="text-black px-2 py-1" />
        <select value={side} onChange={(e) => setSide(e.target.value)} className="text-black px-2 py-1">
          <option value="Home">Home</option>
          <option value="Away">Away</option>
        </select>
        <input type="number" value={round} onChange={(e) => setRound(Number(e.target.value))} className="text-black px-2 py-1" />
        <button onClick={loadTeamData} className="bg-blue-500 px-3 py-1 rounded">Załaduj dane</button>
      </div>

      <h2 className="text-xl mb-4">2️⃣ Wybierz składniki Power Score (łącznie 100%)</h2>
      ...
      <p className="text-sm mt-2">
        💡 Dostępna pula wag: <strong>{(100 - getTotalWeight()).toFixed(1)}%</strong>
      </p>
      ...
      {resultData && (
        <div className="mt-6 p-4 rounded-xl bg-gray-800 text-white border border-green-400">
          ...
          <p className="text-xl mb-2">Wartość: <strong>{resultData.score}</strong></p>
